...
def load_aircraft_model_data(db_cursor, file_path):
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        ...
        for row in csv_reader:
            # Mapping to match the shorter field names in the file
            model_code = truncate_string(row.get('CODE', '').strip(), 7) or None
            if not model_code:
                print("Skipping row due to missing model_code.")
                continue  # Skip this row if model_code is missing

            manufacturer_name = truncate_string(row.get('MFR', '').strip(), 30) or None
            model_name = truncate_string(row.get('MODEL', '').strip(), 20) or None
            type_aircraft = truncate_string(row.get('TYPE-ACFT', '').strip(), 1) or None
            type_engine = truncate_string(row.get('TYPE-ENG', '').strip(), 2) or None
            aircraft_category_code = truncate_string(row.get('AC-CAT', '').strip(), 1) or None
            builder_certification_code = truncate_string(row.get('BUILD-CERT-IND', '').strip(), 1) or None
            number_of_engines = int(row.get('NO-ENG', '').strip() or 0)
            number_of_seats = int(row.get('NO-SEATS', '').strip() or 0)
            aircraft_weight_category = truncate_string(row.get('AC-WEIGHT', '').strip(), 7) or None
            aircraft_cruising_speed = int(row.get('SPEED', '').strip() or 0)
...
def load_engine_data(db_cursor, file_path):
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        csv_reader = csv.DictReader(file)
        for row in csv_reader:
            # Adjust field names to match the ENGINE.txt file header
            engine_code = truncate_string(row.get('CODE', '').strip(), 5) or None
            if not engine_code:
                print("Skipping row due to missing engine_code.")
                continue  # Skip this row if engine_code is missing

            manufacturer_name = truncate_string(row.get('MFR', '').strip(), 50) or None
            engine_model_name = truncate_string(row.get('MODEL', '').strip(), 13) or None
            type_engine = truncate_string(row.get('TYPE', '').strip(), 2) or None
            horsepower = int(row.get('HORSEPOWER', '').strip() or 0)
            pounds_of_thrust = int(row.get('THRUST', '').strip() or 0)
            ...
            db_cursor.execute(
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    pounds_of_thrust = VALUES(pounds_of_thrust)
                """, (engine_code, manufacturer_name, engine_model_name, type_engine,
                      horsepower, pounds_of_thrust)
            )
